...
import { ApiResponse, ProductData, ProductResponse, UseCreateProductReturn, ValidationErrors } from './types';
...
export function useCreateProduct(): UseCreateProductReturn {
  ...
  const createProduct = async (data: ProductData): Promise<ApiResponse<ProductResponse>> => {
    ...
    try {
      ...
      const response = await fetchData<ApiResponse<ProductResponse>>('/api/v1/products', {
        method: 'POST',
      ...
      if (response.status && response.status >= 400) {
        ...
        if (response.status === 401) {
          ...
          // Error de permisos
          setError('No tienes permisos para crear un producto');
        } else {
          // Otro tipo de error
          setError(response.error || 'Ha ocurrido un error al crear el producto');
        }
      } else {

...
import { ApiResponse, UserData, UserResponse, UseCreateUserReturn, ValidationErrors } from './types';
...
export function useCreateUser(): UseCreateUserReturn {
  ...
  const createUser = async (data: UserData): Promise<ApiResponse<UserResponse>> => {
    ...
    try {
      ...
      const response = await fetchData<ApiResponse<UserResponse>>('/api/v1/users', {
        method: 'POST',
      ...
      if (response.status && response.status >= 400) {
        ...
        if (response.status === 401) {
          ...
          // Error de permisos
          setError('No tienes permisos para crear un usuario');
        } else {
          // Otro tipo de error
          setError(response.error || 'Ha ocurrido un error al crear el usuario');
        }
      } else {

...
import { ApiResponse, CompanyData, CompanyResponse, UseCreateCompanyReturn, ValidationErrors } from './types';
...
export function useCreateCompany(): UseCreateCompanyReturn {
  ...
  const createCompany = async (data: CompanyData): Promise<ApiResponse<CompanyResponse>> => {
    ...
    try {
      ...
      if (response.status && response.status >= 400) {
        ...
        if (response.status === 401) {
          ...
          // Error de permisos
          setError('No tienes permisos para crear una empresa');
        } else {
          // Otro tipo de error
          setError(response.error || 'Ha ocurrido un error al crear la empresa');
        }
      } else {
